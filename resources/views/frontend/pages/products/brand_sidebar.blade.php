    ...
    <div class="widget widget-categories">
        ...
        <ul class="cat-list style1 widget-content">
            ...
            <li>
                <span><a href="{{route('product.brand',$product_brand->slug)}}">{{ucwords(@$product_brand->name)}}</a><i>({{@$product_brand->series->count()}})</i></span>
                <ul class="cat-child">
                    @foreach(@$product_brand->series as $series)
                    <li>
                        <a href="{{route('product.brandseries',[@$product_brand->slug,@$series->slug])}}" title="">{{ucwords(@$series->name)}}</a>
                    </li>
                    @endforeach
